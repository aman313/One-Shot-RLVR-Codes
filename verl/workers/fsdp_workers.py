...
# rm -rf verl/workers/fsdp_workers.py; vim verl/workers/fsdp_workers.py
"""
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if role == 'actor':
            print(f"### weight_decay: {optim_config.get('weight_decay', 1e-2)}")
    ...
    #     # Explicitly collect garbage before starting generation
    #     gc.collect()
    ...
    #             # Set reasonable default max_new_tokens if not specified
    #             if 'max_new_tokens' not in prompts.meta_info:
    #                 prompts.meta_info['max_new_tokens'] = 256
                
    #             # Explicitly garbage collect again
    #             gc.collect()
    #             torch.cuda.empty_cache()
                
    #             log_gpu_memory_usage('Before rollout generation', logger=logger)
    ...
    #         # Try to recover by forcing memory cleanup
    #         gc.collect()
    #         torch.cuda.empty_cache()
            
    #         # Return an empty DataProto with error flag if generation fails
    #         error_output = DataProto()
    #         error_output.meta_info = {"error": str(e)}
    #         return error_output
