...
class RayPPOTrainer(object):
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(training_state_path):
            ...
            self.history_accuracy = {int(k): v for k, v in training_state['history_accuracy'].items()}
            if 'history_accuracy_with_step' in training_state:
                self.history_accuracy_with_step = {
                    int(k): {int(step): acc for step, acc in v.items()} 
                    for k, v in training_state['history_accuracy_with_step'].items()
                }
            else:
                self.history_accuracy_with_step = {}
